...
/**
 * 简化{@code R<T>} 的访问操作,来自PIG项目,例子如下
 * <pre>
...
public class RetOps<T> {
    ...
    /** 状态码为成功 */
    public static final Predicate<R<?>> CODE_SUCCESS = r -> RCode.SUCCESS.getStatus() == r.getCode();
     ...
     * @return 返回ture表示相等
     */
    public boolean codeEquals(RCode rCode) {
        return original.getCode() == rCode.getStatus();
    }
    ...
    public boolean isSuccess() {
        return codeEquals(RCode.SUCCESS);
    }
    ...
    public <Ex extends Exception> RetOps<T> assertSuccess(Function<? super R<T>, ? extends Ex> func) throws Ex {
        return assertCode(RCode.SUCCESS.getStatus(), func);
    }
